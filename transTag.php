...
<?php $trans_id=$_GET['id']; if(!session_id()){session_start();} include("conn.php");?>
	...
	if(isset($_SESSION['uid'])){
		$uid=$_SESSION['uid'];
		$sql=mysqli_query($conn,"select * from tb_tk_tag where tag_logord=2 and tag_logid='$trans_id' and tag_uid='$uid'");
		if($info=mysqli_fetch_array($sql)){
	?>
    	<a href="check_dellogTag.php?id=<?php echo $trans_id;?>&state=2">取消点赞</a>
    <?php
		}else{
	?>
    	<a href="check_logTag.php?id=<?php echo $trans_id;?>&state=2">点赞</a>
    <?php 
		}
	}else{
		?>
    ...
    <div class="tagrecord">
    	<?php
		$sql=mysqli_query($conn,"select * from tb_tk_tag join tb_tk_user on tb_tk_tag.tag_uid=tb_tk_user.user_uid where tag_logid='$trans_id' and tag_logord=2 order by tag_time desc");
